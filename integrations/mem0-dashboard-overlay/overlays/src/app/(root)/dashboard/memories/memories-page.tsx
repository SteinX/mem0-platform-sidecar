...
export default function MemoriesPage() {
  ...
  const memoryId = normalizeMemoryId(searchParams.get("memoryId"));
  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      ...
      if (
        mountedRef.current
        && isCurrentMemoryRequest(generation, requestGeneration.current)
      ) {
  ...
  const columns = useMemo<MemoryColumn[]>(() => [
    ...
    },
    {
      ...
      label: "Categories",
      width: 15,
      render: (_value, row) => <MemoryCategories categories={row.categories} />,
    },
    {
      key: "id",
      label: "Action",
  ...
  return (
    <div className="min-w-0 space-y-5">
          ...
          <div className="space-y-2 md:hidden">
            {rows.map((memory) => (
              ...
              >
                ...
                <MemoryCategories categories={memory.categories} mobile />
              </button>
            ))}
          </div>
        </>
      ...
      {pageData && shouldShowMemoryPagination(query.page, pageData.has_more) ? (
        <Pagination>
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
