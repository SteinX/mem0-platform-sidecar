...
def test_scheduler_skips_completed_job_dedupe_in_same_window(db_session) -> None:
    ProjectRepository(db_session).upsert_default_project(
        project_id="repo-a",
        name="Repo A",
        mem0_base_url="http://mem0:8000",
    ...
    run = db_session.query(ConsolidationRun).one()
    job = db_session.query(Job).one()
    ConsolidationRunRepository(db_session).mark_running(
        run.id,
        scan_cutoff=observed,
    )
    ConsolidationRunRepository(db_session).mark_succeeded(
        run.id,
        counts={},
        completed_at=observed,
    )
    JobRepository(db_session).mark_succeeded(job.id, result={})
    db_session.commit()
