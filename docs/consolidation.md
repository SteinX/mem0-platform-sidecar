...
5. Enable `OBSERVE` for seven days; inspect proposal precision and queue latency.
6. Enable `MANUAL`; shadow exact duplicates only and exercise rollback.
7. Enable `AUTO_SAFE` with hard delete still false for fourteen days.
8. Enable hard delete only after zero incorrect-shadow incidents and a
